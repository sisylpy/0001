...
//        map.put("status", status);
        ...
        try {
            ...
            if (nxCommunitySplicingOrdersEntities.size() > 0) {
                for (NxCommunitySplicingOrdersEntity splicingOrdersEntity : nxCommunitySplicingOrdersEntities) {
                    ...
                    if (splicingOrdersEntity.getNxCsoBuyMemberTime() > 0) {
                        String memberTime = formatWhatDay(nxOrders.getNxCoBuyMemberOrderTime() * 30);
    ...
    @ResponseBody
    @RequestMapping(value = "/customerCashPay0", method = RequestMethod.POST)
    public R customerCashPay0(@RequestBody NxCommunityOrdersEntity nxOrders) {
